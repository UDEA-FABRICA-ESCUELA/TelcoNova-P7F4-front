    ...
    return <div className="min-h-screen bg-background p-6">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white rounded-lg border">
            ...
            {Array.from({
            length: 5
          }).map((_, i) => <div key={i} className="border-b p-4 flex justify-between items-center">
...
Lista de Notificaciones</h3>
          ...
          {templates.length === 0 ? <div className="bg-white rounded-lg border p-12 text-center">
              ...
              <Table>
                ...
                <TableBody>
                  {templates.map(template => <TableRow key={template.id} className={cn("border-b hover:bg-muted/50", deleteLoading === template.id && "opacity-50")}>
                      ...
                      <TableCell className="py-4 px-6">
                        <div className="flex items-center gap-2">
                          <Button variant="ghost" size="sm" onClick={() => onEdit(template)} disabled={deleteLoading === template.id} className="p-2 h-8 w-8">
                            <Edit className="h-4 w-4 text-muted-foreground" />
                          </Button>
                          <Button variant="ghost" size="sm" onClick={() => handleDeleteClick(template)} disabled={deleteLoading === template.id} className="p-2 h-8 w-8">
                            {deleteLoading === template.id ? <svg className="h-4 w-4 animate-spin text-muted-foreground" fill="none" viewBox="0 0 24 24">
                                <circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" className="opacity-25" />
                                <path fill="currentColor" d="m4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" className="opacity-75" />
                              </svg> : <Trash2 className="h-4 w-4 text-muted-foreground" />}
                          </Button>
                        </div>
                      </TableCell>
                    </TableRow>)}
      ...
      <ConfirmationDialog open={deleteDialog.open} onOpenChange={open => setDeleteDialog({
      open,
      template: null
    })} title="Eliminar Plantilla" description={deleteDialog.template ? `¿Estás seguro de que deseas eliminar la plantilla "${deleteDialog.template.name}"? Esta acción no se puede deshacer.` : ""} confirmText="Eliminar" cancelText="Cancelar" variant="destructive" onConfirm={handleDeleteConfirm} loading={!!deleteLoading} />
    </div>;
}